...
class VideoAnalyzer:
    ...
    def request_transcript(self):
        ...
        if "transcript_data" in self.data.keys():
            if not self.data["transcript_data"]["is_whisper_transcribed"] and not len(self.data["transcript_data"]["text"]):
                raise Exception("There is still no transcript, the video has been added to the transcription queue but is still not available.")
            return
        
        # Transcripts are disabled
        try:
            YTTranscriptApi.list_transcripts(self.video_id)
        except Exception as e:
            ...
            raise Exception("There is no transcript, the video will been added to the transcription queue but is not immediately available, come back later!")
